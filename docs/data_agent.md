# data-agent（数据 Agent（代理））作业手册

## Role Scope（职责范围）
- Schema（结构规范）定义与维护
- Qwen-VL（多模态模型）提示词与结构化输出约束
- 契约变更流程维护（CHANGE_LOG/INTERFACES）

## Allowed Paths（允许修改的路径）
- `docs/schemas/`
- `docs/coordination/INTERFACES.md`
- `docs/coordination/CHANGE_LOG.md`
- `docs/data_agent.md`
...
2. 所有契约变更必须先记录到 `docs/coordination/CHANGE_LOG.md`
3. 更新 `docs/coordination/INTERFACES.md` 并同步相关 Agent（代理）
4. 任何关键决策写入 `docs/coordination/DECISION_LOG.md`

## Round Flow（轮次流程）
1. 确认 Scope（范围）与 DoD（完成定义）
2. 实现 → Unit Test（单元测试）/Schema 校验 → 记录结果
...
- Schema（结构规范）是 SSOT（单一事实源）
- 任何契约变更以 `docs/coordination/CHANGE_LOG.md` 与 `docs/coordination/INTERFACES.md` 为准

## Test Commands（测试命令）
- TBD（待定）：在首次可运行 Schema 校验后写入本节

## Quality Gates / DoD（质量门禁/完成定义）
- Scope（范围）完成且无越界修改
- Schema 校验/Unit Test（单元测试）通过并记录
- 契约变更已更新 `CHANGE_LOG.md` 与 `INTERFACES.md`
...
## Non-goals（非目标）
- 不修改 `client/`、`core/`、`server/` 代码（除非 Round（轮次）明确要求）
