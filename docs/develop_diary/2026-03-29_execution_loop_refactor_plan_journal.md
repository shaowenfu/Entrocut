...
今天我重新回头看了 `core/main.py`，比起继续纠结局部字段，我更清楚地看到真正的卡点其实只有一个：
...
`上下文编排不能继续留在 core/main.py 里和 API、状态、任务逻辑混在一起。`
...
`从这一步开始，上下文优化应该优先发生在 context_engineering 模块里，而不是回到 main.py 继续堆变量和字符串。`
...
因为它们各自都有自己的主责边界，但又能在 `core/main.py` 和 `core/context_engineering.py` 这两个关键位置自然接上。
