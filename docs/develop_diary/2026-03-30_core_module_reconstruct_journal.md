...
今天我把 `core/main.py` 这块长期悬着的技术债，真正收了一次口。

这次工作的目标不是“把一个大文件拆小一点”，而是把 `core` 里的职责边界重新摆正。

在重构之前，`core/main.py` 同时承载了太多东西：
...
2. `main.py` 在装配阶段注入 `lambda: AGENT_LOOP_MAX_ITERATIONS`
...
## 三、这次拆分之后，main.py 终于像 main.py 了

重构前的 `main.py` 是所有东西的入口和实现。
...
也就是说，测试会把 `main.py` 当成一个动态加载模块，并直接拿它上面的 `json` 模块引用。

这件事从工程美学上看当然不完美，但在当前仓库里它就是一个已有 surface。

所以我没有强行“清理得很纯”，而是继续在 `main.py` 里保留了：
