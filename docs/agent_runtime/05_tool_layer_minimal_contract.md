...
深入理解已知 `clip`，或对少量候选做更深判断、比较、消歧和重排。

它回答的是：

`这个 clip 画面里到底有什么，或这些候选里谁最适合当前目标。`
...
`retrieve` 解决“找得到”，`inspect` 同时解决“看得懂”和“选得准”。

当前阶段说明：

1. `describe` 用于让文本 `Agent` 深入理解一个已知 `clip`
2. `verify / compare / choose / rank` 用于候选判断
3. 当用户指定 `clip` 或视觉细节不确定时，`inspect` 可以不经过新的 retrieval 步骤直接调用
4. 第一版 `describe` 复用本地 stitched keyframes 作为视觉证据
