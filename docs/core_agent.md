# core-agent（算法 Agent（代理））作业手册

## Role Scope（职责范围）
- Python（解释语言）本地算法 Sidecar（本地算法服务）
- 镜头切分与抽帧（PySceneDetect/FFmpeg）
- 与 client 的 JSON-RPC（JSON 远程过程调用）接口
...
2. 推送到远程仓库：`git push origin core-agent`
...
- Schema（结构规范）：`docs/schemas/analysis.json`（如有消费）
...
## Non-goals（非目标）
- 不修改 `client/` 与 `server/` 代码（除非 Round（轮次）明确要求）
