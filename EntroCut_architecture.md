...
- **职责范围**：
    - **UI 渲染**：负责所有界面的绘制，通过数据驱动（Data-Driven）展示当前的素材、分镜和对话。
    - **用户意图捕获**：监听用户的输入（Prompt）、拖拽文件夹、手动调整时间轴顺序。
...
- **职责范围**：
    ...
    - **Agent 逻辑编排**：运行逻辑循环（如 ReAct 模式）。决定下一步是去调用 `Server` 进行语义检索，还是调用 FFmpeg 修改视频片段。
...
4. **Core**：
    ...
    - 根据返回的片段 ID，自主决策分镜顺序（Storyboard）。
    - 通过 WebSocket 把更新后的 `storyboard` 数据推回 **Client**。
5. **Client**：Zustand 监听到数据变化，界面自动弹出分镜卡片。
