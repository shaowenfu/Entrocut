...
`视频剪辑的本质不是页面层的时间线操作，而是围绕用户 intent 对候选片段做持续收敛。`

因此 `Core` 之所以必须存在，不是因为前后端分层习惯，而是因为系统需要一个离素材最近、又能持续维护 `EditDraft` 与上下文状态的本地中枢。

- **职责范围**：
    - **上下文工程 (Context Engineering)**：从当前工程状态中裁剪出当前一步决策真正需要的高信号上下文，而不是机械拼一个巨大的 `Long Context Prompt`。
...
2. **Core**：
    - 读取当前 `EditDraft`、局部 `target`、最近工作记忆和本地索引。
    - 组装当前一步决策所需的最小上下文，而不是全量工程状态。
    - 发给 **Server** 换取下一步结构化决策。
3. **Server**：验证用户权限，转发给大模型，返回“搜索关键词：skipping, jumping, snow spray”。
4. **Core**：
    - 拿到关键词，生成搜索向量。
    - 再次请求 **Server** 调用 DashVector 检索。
    - 基于检索结果执行 `select -> compose -> evaluate -> revise` 中当前这一轮最合适的一步。
    - 更新 `EditDraft`，并把新状态推回 **Client**。
...
- **第一性原理一致**：系统真实在做的是“围绕 intent 持续收敛 EditDraft”，不是把时间线编辑器远程化。
