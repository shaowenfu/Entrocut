...
用户不是来“编辑时间线控件”的，而是来表达一个剪辑意图。

系统真正要帮用户做的是：

`把模糊 intent 逐步收敛成一个可执行的 EditDraft`

因此所有用户场景，本质上都在围绕这 4 个动作循环：
...
* **上下文构建**：Core 读取当前 `EditDraft`、可用 `clips`、最近对话摘要和当前 `target`。
* **逻辑编排**：Core 不是要求模型“一次性剪完”，而是要求它给出当前这一步最合适的决策。
* **中转调用**：Core 请求 Server 转发给大模型。
...
4. **生成决策 (Core)**：
* Core 根据检索结果，自主决定当前是先扩召回、先精判，还是直接更新 `EditDraft`。
* Core 生成新的 `shot / scene / EditDraft` 结构，而不是先退回到展示型 `Storyboard`。
...
* **Core**: 监听到 Client 发来的顺序变更，实时更新本地 `EditDraft` 中的 `shot / scene` 顺序。
...
| --- | --- | --- | --- |
| **1. 导出指令** | 点击 Export，界面进入 `isEditLocked` 状态 | 锁定任务流，读取最终 `EditDraft.shots` 序列 | 无感知 |
| **2. 渲染合成** | 显示百分比进度条 | **[重活]** 调用 FFmpeg 按照 `EditDraft` 的入点出点进行视频切片、拼接、添加转场 | 无感知 |
